...
float side12[] = new float[41];
int traceMatrix[][] = new int[41][41];
...
//servo alignment
static int s1offset = 0;
static int s2offset = 0;
static int s3offset = 0;
...
void setup() {
  ...
  //Draw Square Button
  controlP5.addButton("sketch").setPosition(20, 70).setSize(50, 20); 
  
  //Draw Trace Matrix Button
  controlP5.addButton("raster").setPosition(20, 100).setSize(50, 20); 
  
  //Draw Trace Matrix Button
  controlP5.addButton("zigzag").setPosition(20, 150).setSize(50, 20); 
...
{   
    for(int i=0;i<41;i++)
    {
       ...
       for(int j=0;j<41;j++)
       {
         ...
         //println("Calculating theta: Step"+i+"XYZ:  "+xp+" "+yp+" "+zp);
         print(i+""+j + " ");
         setThetasfromXYZ();
         //delay(100);
       }        
...
void SetServos() {
  if ( debug ) { println("Entering: SetServos()"); }
    ...
    //println("Sending from Set Servos   t1:"+ t1+"  \t t2:"+t2+" \tt3: "+t3);
...
{
  if ( debug ) { println("Entering sendMove"); }

  //println("Writing to Servos"+"servo "+ servo+"val "+val);

  myPort.write(255);
  myPort.write(servo);
  myPort.write(val);
  
 if ( debug ) { println("Exiting sendMove"); }
...
{
  int j = 20;
  //float xArray[] = new float[40];
  //float yArray[] = new float[40];
